...
class ImgData(pl.LightningDataModule):
    """ This class handles the Training and Validation dataset 
        for data saved in a folder in the current working directory. 
        The Dataset folder is expected to contain images classified subfolders 
        The number of subfolders indicates the number of classes while 
        the name of the subfolders are the classnames """
    ...
    def prepare_data(self):
        pass
    def myDataLoader(self,data_val,batch_size,num_workers,pin_memory):
        with FileLock("./Sort-Dataset.lock"):
            data=DataLoader(data_val, batch_size=batch_size, num_workers=num_workers, pin_memory=pin_memory)
        return data

    def setup(self, stage: str = 'fit'):
        data_full, _ = self.dataset()
        self.data_train, self.data_val, self.data_test = random_split(data_full, [int(round(
            len(data_full)*0.7)), int(round(len(data_full)*0.2)), int(round(len(data_full)*0.1))])

    def train_dataloader(self):
        return self.myDataLoader(self.data_train, batch_size=self.batch_size, num_workers=self.num_workers, pin_memory=False)

    def test_dataloater(self):
        return self.myDataLoader(self.data_test, batch_size=self.batch_size, num_workers=self.num_workers, pin_memory=False)

    def val_dataloader(self):
        return self.myDataLoader(self.data_val, batch_size=self.batch_size, num_workers=self.num_workers, pin_memory=False)
...
class ImageFolderNp(datasets.ImageFolder):
    ...
    def __init__(self,
        ...
        # loader, IMG_EXTENSIONS if is_valid_file is None else None,
        super(ImageFolderNp, self).__init__(root)
        # transform=transform,
