...
def train_fn(config, model_arch, data_dir=os.path.join(os.getcwd(), "Dataset") , num_epochs=60, num_gpus=0, checkpoint_dir=None):
   dm = datamodule.ImgData(num_workers=1, batch_size=config["batch_size"],data_dir=data_dir)
   model = model_name(model_arch)(config,  dm.num_classes)
   metrics = {"loss": "ptl/val_loss", "acc": "ptl/val_accuracy"}
   pl.seed_everything(42, workers=True)
   comet_logger=CometLogger(api_key = key(), experiment_name =model_arch, project_name = "deep-tuner-stage-2")
   trainer = pl.Trainer(
      max_epochs=num_epochs,
      gpus=num_gpus,
      logger = [comet_logger,TensorBoardLogger(save_dir=tune.get_trial_dir(), name="my_model")],
